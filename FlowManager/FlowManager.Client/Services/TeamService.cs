...
namespace FlowManager.Client.Services
{
    public class TeamService
    {
        ...
        public async Task<ApiResponse<PagedResponseDto<TeamResponseDto>>> GetAllModeratorTeamsByStepIdAsync(Guid stepId, QueriedTeamRequestDto payload)
        {
            try
            {
                UriBuilder uriBuilder = new UriBuilder(_httpClient.BaseAddress!)
                {
                    Path = $"api/teams/moderators/queried/{stepId}"
                };
