...
namespace FlowManager.Client.Services
{
    public class UserService
    {
        ...
        public async Task<ApiResponse<PagedResponseDto<UserResponseDto>>> GetAllUsersByStepQueriedAsync(Guid stepId, QueriedUserRequestDto? payload = null)
        {
            try
            {
                var uriBuilder = new UriBuilder(_httpClient.BaseAddress!)
                {
                    Path = $"api/users/queried/byStep/{stepId}"
                };
